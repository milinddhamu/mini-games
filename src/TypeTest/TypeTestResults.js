...
export default function TypeTestResults({data,handleRestart}){
  const resultMetrics = [
    { name: "Correct Words", data: data?.correctWords,color:"success",textColor:"green"},
    { name: "Incorrect Words", data: data?.incorrectWords,color:"danger",textColor:"red" },
    { name: "Skipped Words", data: data?.skippedWords,color:"",textColor:"slate" },
    { name: "Extra Words", data: data?.extraWords,color:"warning",textColor:"yellow" },
  ];
    ...
    <div className="flex flex-col justify-center items-center w-full gap-4 pt-6">
      ...
      <div className="flex flex-col justify-center items-center w-full max-w-fit p-4 px-8 gap-4">
        <div className="flex flex-row w-full justify-center items-end gap-1">
          <Counter from={0} to={data.wordsPerMinute}/>
          <h1 className="text-3xl font-bold tracking-tightest">WPM</h1>

        </div>
        <h2 className="text-lg tracking-widest uppercase opacity-70">at {data.accuracy}{"%"} accuracy</h2>
      ...
      <button onClick={handleRestart} className="px-20 py-14 m-8 border-1 border-gray-500/50 group"><TiRefresh  className="text-5xl opacity-40 group-focus:opacity-60 group-focus:rotate-45 transition-all ease-in duration-150"/>
      </button>
      </Tooltip>
    </div>
    </>
